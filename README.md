...
- вход по логину и паролю, bootstrap первого админа через `.env`;
- админка `/admin/users` для создания, отключения пользователей и сброса паролей;
- персональные контексты, избранное и скрытые объекты для каждого пользователя;
- список объявлений из текущей БД;
...
- сортировка `Лучшие` по recommendation score;
- статистика по каждому объявлению: наблюдения, мин/макс цена, изменение с первого наблюдения;
- карточка объявления с историей наблюдений, изменениями цены и объяснением score.

Score считается из текущей базы: сравнение цены за м² с медианой сегмента
`район + диапазон площади`, этаж, площадь, возраст объявления и динамика цены.

Для первого запуска задайте в окружении web-сервиса `APP_ADMIN_USERNAME`,
`APP_ADMIN_PASSWORD` и, при необходимости, `APP_ADMIN_DISPLAY_NAME`.
